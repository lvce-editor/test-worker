...
test('toHaveText: ok path', async () => {
  const mockInvoke = jest.fn<(...args: readonly any[]) => Promise<any>>().mockResolvedValue({})
...
test('toHaveText: error path builds ConditionErrors', async () => {
  const mockInvoke = jest.fn<(...args: readonly any[]) => Promise<any>>().mockResolvedValue({ error: true })
  const mockRpc = MockRpc.create({ commandMap: {}, invoke: mockInvoke })
  RendererWorker.set(mockRpc)
